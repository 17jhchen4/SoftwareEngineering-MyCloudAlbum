...
public class ImgUtil
{
    ...
    //4.4及以上系统使用这个方法处理图片
    @TargetApi(19)
    public static String handleImageOnKitKatNoCompress(Context context, Intent data)
    {
        String imagePath = null;
        Uri uri = data.getData();
        if (DocumentsContract.isDocumentUri(context, uri))
    ...
    //4.4以下系统使用这个方法处理图片
    public static String handleImageBeforeKitKatNoCompress(Context context, Intent data)
    {
        Uri uri = data.getData();
        String imagePath = getImagePath(context, uri, null);
        return imagePath;
